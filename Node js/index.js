...
//express is used for making REST API- representational state transfer application programming interface
const cors = require("cors");
//cross-origin resource sharing - used to share resources b/w frontend and backend
const app = express();
app.use(cors());
app.get("/search", (request, response) => {
  const query = request.query.input;
  response.send(searchKeywords.get(query));
});
...
documents.forEach((doc) => {
  const words = doc.content.split(" "); // Step 1: Split the document text by spaces into individual words, character string get splitted based on word space,
  //ex: I AM DISHA ---> O/P : {"I","AM","DISHA"}
  words.forEach((word) => {
    const Lower = word.toLowerCase();
    //toLowerCase to avoid case sensitive issue
    if (!searchKeywords.has(Lower)) {
      // Step 2: Check if the word is already a key in the Map
      searchKeywords.set(Lower, []); // Step 3: Initialize with an empty array if it's a new word, otherwise it leads to duplicate string
    }
    searchKeywords.get(Lower).push(doc); // Add the document to the array for this word
  });
});
